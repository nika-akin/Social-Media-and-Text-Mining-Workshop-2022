...
```
- **? Welche Implikationen im Forschungszyklus haben Ihre Entscheidungen hier bei der Formulierung der Query?**
...
```
**Verteilung der täglichen Tweets**
```{r First glimpse Verteilung Tweet Anzahl}
ggplot(data = tweets,
      aes(x = tweetcreatedts)) +
      geom_histogram(bins = 57, color = "black", fill = NA)+
      ggtitle(label = "Tägliche Tweetanzahl")
```
**Wrangling der Datums-Zeitvariablen mit [lubridate](https://lubridate.tidyverse.org/)**
```{r Wrangling Zeitvariablen}
tweets <- tweets %>%
  mutate(hour = floor_date(tweetcreatedts, 
                           unit= "hour")) #Runden von date-time Objekten (Sekunde, Woche, Monat)

```
**Stündliche Tweets und Retweets** 
```{r}
hour_summary <- tweets %>%
  group_by(hour) %>%
  summarize(tweets = n(),
